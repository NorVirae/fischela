...
const BuyOrders = (props) => {
    ...
    return <div className="buy-order">
            ...
            <div className=''>
                <h4 className={''}>Buy Orders</h4>

                <div className="grid grid-5 gap-1"> 
                    {/* cols */}
                   
                    {orders && orders.length != 0 ? orders.map((ord, index) => (<div onClick={e=>props.history.push("/d/order/preview/"+index)} className="order-sum card flex flex-column p-1">
                       ...
                       <button onClick={e=>props.history.push("/d/order/preview/"+index)} className="col-lg-12 btn-cart">View Order</button>
